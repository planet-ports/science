...
;;; -------------------------------------------------------------------
;;;
;;; This module tests most of the science collection functionality.
...
(let ((h (make-discrete-histogram)))
  (for ((i (in-range 10000)))
    (discrete-histogram-increment! h (random-bernoulli .6)))
...
(define (chebyshev-example n)
  (let ((cs (make-chebyshev-series f 40 0.0 1.0))
        (y-values '())
        (y-cs-10-values '())
        (y-cs-40-values '()))
